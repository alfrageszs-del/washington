-- =====================================================
-- ПРИМЕНЕНИЕ ENUM'ОВ К ТАБЛИЦАМ WASHINGTON (ИСПРАВЛЕННАЯ ВЕРСИЯ)
-- =====================================================
...
DO $$ 
BEGIN
    -- Проверяем, что колонка существует и имеет тип TEXT
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE verification_requests ADD COLUMN target_faction_new faction_enum;
            UPDATE verification_requests SET target_faction_new = target_faction::faction_enum WHERE target_faction IS NOT NULL;
            ALTER TABLE verification_requests DROP COLUMN target_faction;
            ALTER TABLE verification_requests RENAME COLUMN target_faction_new TO target_faction;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE verification_requests ADD COLUMN target_department_new department_enum;
            UPDATE verification_requests SET target_department_new = target_department::department_enum WHERE target_department IS NOT NULL;
            ALTER TABLE verification_requests DROP COLUMN target_department;
            ALTER TABLE verification_requests RENAME COLUMN target_department_new TO target_department;
