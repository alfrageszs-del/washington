...
export default function CourtActsPage() {
  const [acts, setActs] = useState<CourtAct[]>([]);
  ...
  const loadUserAndActs = async () => {
    try {
      ...
      const { data: actsData, error: actsError } = await supabase
        ...
        .select(`
          *,
          judge:profiles!court_acts_judge_id_fkey(full_name)
        `)
        .eq("status", "published")
      ...
      if (actsError) {
        ...
        const formattedActs = actsData?.map(act => ({
          ...act,
          judge_name: act.judge?.full_name || "Неизвестно"
        })) || [];
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Судебные акты</h1>
              <p className="mt-2 text-sm text-gray-600">
                Реестр всех опубликованных судебных актов
              </p>
            </div>
            {canCreate && (
              <button
        ...
        {acts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Пока не создано ни одного судебного акта.
            </p>
          </div>
        ) : (
                ...
                <div className="p-6">
                  ...
                  <div className="border-t pt-4">
                    <div className="flex items-center justify-between text-sm text-gray-500">
                      <span>Автор: {act.judge_name}</span>
      ...
      {showCreateForm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">Создать новый судебный акт</h3>
                <button
